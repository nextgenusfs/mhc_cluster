...
print "CMD: Runner HMMER3 using MHC DRB HMM model (using %s cpus)\n" % cpus
hmm_out = args.out + '.DRB.hmm.txt'
hmm = script_path + '/lib/MHC_DNA.hmm'
FNULL = open(os.devnull, 'w')
subprocess.call(['hmmscan', '--cpu', cpus, '--domtblout', hmm_out, hmm, sort_out], stdout = FNULL, stderr = FNULL)
...
filtered = SeqIO.parse(sort_out, "fasta")
for rec in filtered:
    if rec.id in q_list:
        L = len(rec.seq)
        if L < avg_len:
